...
let nomors = m.sender
  ...
  if (10000 > users[who].atm) throw 'Target Gaada atm bodoh, Kismin dia'
  users[who].atm -= dapat * 1
  users[m.sender].atm += dapat * 1
  global.db.data.users[m.sender].lastrob = new Date * 1
  conn.reply(m.chat, `Berhasil Merampok atm Target Sebesar ${dapat}`, m)

}else conn.reply(m.chat, `Anda Sudah merampok dan berhasil sembunyi , tunggu ${timers} untuk merampok lagi`, m)
}
handler.help = ['merampok atm <@tag>']
handler.tags = ['rpg']
handler.command = /^rampokatm$/
handler.limit = true
handler.rowner = true
